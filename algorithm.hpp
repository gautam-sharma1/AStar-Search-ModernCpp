...
public:
    ...
    /*
    Checks if the move is valid and implemets the move
    */
    void Movement(PointI &p)
    {
        ...
        if (IsLegal(p))
        {
            ...
            for (auto &x : _moves) // For moves in x direction
            {
                for (auto &y : _moves) // For moves in y direction
                {
                    if (abs(x) != abs(y)) // Can't move diagonally
                    {
    ...
    /*
    Checks if the move is within bounds. Boundary is defined from [0,10]
    */
    ...
    void SetObstacle(){}; // Setter function
    void GetObstacle(){}; // Getter function
    
    // Implements the main algorithm and calls openCV functions
    void Implementation()
    {
